<template>
  <div>
    <div class="return-notice_content writ-overflow">
      <el-form ref="notice" :model="notice" :rules="rules" label-width="160px" label-position="right"
        :show-message="false">
        ...
        <el-form-item label="" class="mt5" prop="remark">
          ...
          <el-input v-model="notice.remark" class="vab" type="textarea" :autosize="{minRows: 1}" placeholder="备注">
          </el-input>
        </el-form-item>
      </el-form>
    </div>
    ...
    <div class="dialog-footer" style="text-align: right">
      ...
      <el-button type="primary" @click="sure">保存并生成强制执行申请书送达回执</el-button>
...
<script>
  ...
  import { sealWrit, getWritInfo } from "@/api/fileRelated";
        ...
        let inputInfo = this.info.writInfo.reduce((total, item) => {
          switch (item.tagName) {
            ...
            case 'entity1-col2':
              ...
              break
            case 'entity1-col3':
              total.date = item.tagContent ? stringToDate(item.tagContent.replace("年", "-").replace("月", "-").replace("日", "")) : ''
              break
            case 'entity1-col4':
