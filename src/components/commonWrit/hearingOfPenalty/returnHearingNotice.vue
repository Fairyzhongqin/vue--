<template>
  <div>
    ...
    <div class="dialog-footer" style="text-align: right">
      ...
      <el-button type="primary" @click="sure">保存并生成行政处罚听证会通知书送达回执</el-button>
...
import { sealWrit, getWritInfo, getWritDataByWritId } from '@/api/fileRelated'
export default {
  props: ['info'],
  data() {
    return {
  ...
  methods: {
      ...
      this.$refs.notice.validate((val, errInfo) => {
        if (val) {
          ...
          let params = {
            ...
            writType: 'XZCFTZHTZSSDHZ',
            sourceId: this.info.caseId, // LA
            data: data
          }
    ...
    // 初始化文书
    initInfo() {
      this.info.brothers.map(item => {
        if (item.writTypeName == 'XXCFTZHTZS') {
          this.notice.name = item.writNoName
        }
      })
      let user = JSON.parse(localStorage.getItem('res'))
      this.notice.unit = this.info.partyName
      this.notice.caseName = this.info.caseName
      //  + "案";
      this.notice.address = ''
      this.notice.server = ''
    }
