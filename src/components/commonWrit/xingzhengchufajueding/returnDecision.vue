<template>
  <div class="return-decision_content">
    <el-form ref="notice" :model="notice" :rules="rules" label-width="160px" label-position="right" :show-message="false">
      ...
      <el-form-item label="" prop="writNo">
      ...
      <div v-for="(item, index) in notice.dataInfo" :key="index">
        <el-row :gutter="20">
          <el-col :span="12">
            <el-form-item label="" :rules="rules.name" :prop="`dataInfo.${index}.name`">
        ...
        <el-row :gutter="20">
          <el-col :span="12">
            <el-form-item label="" :rules="rules.address" :prop="`dataInfo.${index}.address`">
      ...
      <el-form-item label="" prop="remark">
        ...
        <el-input v-model="notice.remark" placeholder="备注"></el-input>
      </el-form-item>
    </el-form>
    <div class="dialog-footer" style="text-align: right">
      <el-button @click="close">取 消</el-button>
      <el-button type="primary" @click="sure">保存并生成行政处罚听证告知书送达回执</el-button>
...
export default {
  ...
  data() {
    return {
      rules: {
        ...
        unit: [{ required: true, trigger: 'blur', message: '请输入受送达单位（个人）' }],
        name: [{ required: true, trigger: 'blur', message: '请输入送达文书名称、文号' }],
        way: [{ required: true, trigger: 'blur', message: '请选择送达方式' }],
        address: [{ required: true, trigger: 'blur', message: '请输入送达地点' }],
        date: [{ required: true, trigger: 'blur', message: '请选择送达日期' }]
      },
      lawDisabled: false,
      notice: {
        caseName: '', //案件名称
  ...
  methods: {
    ...
    sure() {
      this.$refs.notice.validate((valid, errInfo) => {
        if (valid) {
    ...
    reflect() {
      let entityArr = [],
        fieldArr = ['name', 'address', 'date', 'way', 'people', 'server']
    ...
    initInfo() {
      this.info.auditInfo.docList.map(item => {
        if (item.writTypeName === 'XXCFJDSDW' || item.writTypeName === 'XXCFJDSGR' || item.writTypeName === 'BYXZCFJDS') {
