<template>
  <div>
    <div class="content-list_handlePage_wrapper">
      <!-- 头部开始 -->
      <div class="content-list_handlePage">
        <el-row>
          ...
          <el-col :span="7">
            <div>执法人员：{{ option.examineInfo.executorName }}</div>
          </el-col>
        </el-row>
        <el-row>
      ...
      <div v-show="originProbList && originProbList.length > 0">
        <div v-for="(tips, index) in originProbList" :key="tips.id" class="first_content">
          <div class="first-content-type left-indent">检查项目类别：{{ tips.discretionTypeName }}</div>
          <div class="first-content-primary-tip">
            <el-checkbox
              ...
              :disabled="
                flag === 'flagDetail' ||
                  (tips.isReview === '0' && tips.isQualified === '0') ||
                  (tips.isReview === '1' && tips.isDelay === '2') ||
                  (tips.isReview === '1' && tips.isDelay === '3')
              "
            ...
            <div v-for="(item, sonIndex) in tips.children" :key="item.id">
              <div class="second-content-primary-item">
                <el-checkbox
                  ...
                  :disabled="
                    flag === 'flagDetail' ||
                      (item.isReview === '0' && item.isQualified === '0') ||
                      (item.isReview === '1' && item.isDelay === '2') ||
                      (item.isReview === '1' && item.isDelay === '3')
                  "
              ...
              <!-- flagsee 是复查页面  flagDetail 是详情页面 -->
              <div v-if="flag === 'flagSee' || flag === 'flagDetail'">
                <div class="input-des left-indent noSureLogo">
                  <span style="line-height: 32px">整改时限：{{ !item.expireTime ? '暂无' : item.expireTime }}</span>
                </div>
                <!-- 已复查 不可以操作  -->
                ...
                <div v-if="item.isReview === '1' && item.isDelay === '1'">
                  <div class="input-des left-indent">
                    ...
                    <el-date-picker
                      ...
                      v-if="flag === 'flagSee'"
                    ></el-date-picker>
                    <span v-if="flag === 'flagDetail'">暂无</span>
                  </div>
                  <div class="input-des left-indent">
                    <span class="reviewCheckSure">复查意见：</span>
                    <el-input placeholder="请输入复查意见" v-model="item.reviewOpinion" type="textarea" :autosize="true" v-if="flag === 'flagSee'"></el-input>
                    <span v-if="flag === 'flagDetail'">暂无</span>
                  </div>
                </div>
                <!-- 延期中 不可以操作 -->
                <div v-if="item.isReview === '1' && item.isDelay === '2'">
                  <div class="input-des left-indent">
                    <span class="isDelay">&nbsp; 延期中</span>
                  </div>
                </div>
                <!-- 延期通过 不可以操作 -->
                <div v-if="item.isReview === '1' && item.isDelay === '3'">
                  <div class="input-des left-indent">
                    <span class="isDelay">&nbsp; 延期通过</span>
                  </div>
                </div>
              ...
              <div class="conclusion">
                ...
                <span v-if="item.isReview === '1' && item.isDelay === '1'">
                  ...
                  <span v-if="flag === 'flagDetail'">暂无</span>
                  &nbsp; &nbsp; &nbsp; &nbsp;
                  <i class="isReviewYet" v-if="item.isReview === '1' && item.isDelay === '1'">未复查</i>
                ...
                <span v-if="item.isReview === '0'">
                  ...
                  &nbsp; &nbsp; &nbsp; &nbsp;
                  <i class="isReviewAlready" v-if="item.isReview === '0'">已复查</i>
                  <i class="isRectifyAlready" v-if="item.isRectify === '1'">责令限期整改</i>
                </span>
                <span class="label" @click.prevent="getIndex(item.id)" v-if="flag === 'flagSee' && item.isReview === '1' && item.isDelay === '1'"
                  >上传证据</span
                >
                <!-- @click.prevent="getIndex(item.discRulesId)" -->
              </div>
              <div v-if="item.isQualified === '1'" class="conclusion">
                <span v-if="item.isRectify === '1'">新整改时限：{{item.notQuaExpireTime ? item.notQuaExpireTime : '暂无'}}</span>
                <span v-if="item.isRectify === '0'">
                  新整改时限：
                  <el-date-picker type="date" v-model="item.notQuaExpireTime" placeholder="选择整改期限" value-format="yyyy-MM-dd HH:mm:ss"></el-date-picker>
                </span>
              </div>
              <div class="handle-state left-indet">
          ...
          <!-- 没有子项时的检查项 -->
          <div v-if="!tips.children || tips.children.length === 0" class="left-indent">
            <div style="line-height: 32px">检查描述：{{ tips.negativeDesc }}</div>
          </div>
          <!-- flagSee 是复查页面  flagDetail是详情页面  整改时限都要显示 -->
          <div v-if="(!tips.children || tips.children.length === 0) && (flag === 'flagSee' || flag === 'flagDetail')">
            <div class="input-des left-indent noSureLogo">
              <span style="line-height: 32px">整改时限：{{ !tips.expireTime ? '暂无' : tips.expireTime }}</span>
            </div>
            <!--  isReview === '0' 已复查 不可操作 -->
            ...
            <div v-if="tips.isReview === '1' && tips.isDelay === '1'">
              ...
              <div class="input-des left-indent">
                <span class="reviewCheckSure">复查意见：</span>
                <el-input placeholder="请输入复查意见" v-model="tips.reviewOpinion" autosize type="textarea" v-if="flag === 'flagSee'"></el-input>
                <span v-if="flag === 'flagDetail'">暂无</span>
              </div>
            </div>
            <!-- 延期中 不可操作 -->
          ...
          <!-- 复查结论 分情况 -->
          <div v-if="!tips.children || tips.children.length === 0" class="conclusion">
            <!-- 1.=======>  已复查  isReview === '0'-->
            <span v-if="tips.isReview === '0'">
              复查结论: &nbsp;{{ tips.isQualified === '0' ? '合格' : tips.isQualified === '1' ? '不合格' : '复查结论出错，请联系管理员' }}
              &nbsp; &nbsp; &nbsp; &nbsp;
              <i class="isReviewAlready" v-if="tips.isReview === '0'">已复查</i>
              <i class="isRectifyAlready" v-if="tips.isRectify === '1'">责令限期整改</i>
            </span>
            <!-- 2.=======> 未复查  isReview === '1' && tips.isDelay === '1'-->
            <span v-if="tips.isReview === '1' && tips.isDelay === '1'">
              ...
              <span v-if="flag === 'flagDetail'">暂无</span>
              &nbsp; &nbsp; &nbsp; &nbsp;
              <i class="isReviewYet" v-if="tips.isReview === '1'">未复查</i>
              <i class="isReviewQuestion" v-if="!tips.isReview">复查状态出错，请联系管理员</i>
            </span>
            <span class="label" @click.prevent="getIndex(tips.id)" v-if="flag === 'flagSee' && tips.isReview === '1' && tips.isDelay === '1'">上传证据</span>
            <!-- @click.prevent="getIndex(tips.discId)" -->
          </div>
          <!-- 复查不合格要选择新的整改时限 -->
          <div v-if="tips.isQualified === '1' && (!tips.children || tips.children.length === 0)" class="conclusion">
            <span v-if="tips.isRectify === '0'">
              新整改时限：
              <el-date-picker type="date" v-model="tips.notQuaExpireTime" placeholder="选择整改期限" value-format="yyyy-MM-dd HH:mm:ss"></el-date-picker>
            </span>
            <span v-if="tips.isRectify === '1'">新整改时限：{{tips.notQuaExpireTime ? tips.notQuaExpireTime : '暂无'}}</span>
          </div>
    ...
    <div class="total-proof-list_handlePage">
      <div v-if="proofList && proofList.length > 0" class="proof-list_handlePage">
        <div v-for="(item, index) in proofList" :key="index" class="proof_handlePage">
          <down-proof :proof="item" :canDel="flag != 'flagDetail'" @removeProof="removeProof" type="review"></down-proof>
        </div>
      </div>
      <div class="check-proof_handlePage" v-if="flag === 'flagSee'">
        <upload-proof
          ref="proof"
          @getUploadMsg="getMsg"
          :fileDetail="fileData"
          :uploadType="'rectification'"
          v-bind="$attrs"
...
<script>
import { showRectificationReviewDetail, saveRectificationReview } from '@/api/rectification/index'
import { handleDateRef } from '@/utils/util'
...
export default {
  data() {
    return {
      ...
      writId: null, // 文书id
      fileData: {
        sourceId: this.$store.state.detailId,
        type: '0'
      }, // 设置上传检查项证据材料的参数
      publicProof: [], // 公共证据
      // 禁止选择当前时间之前的检查时间
  ...
  methods: {
    ...
    getMsg(uploadInfo) {
      ...
      if (type === '0') {
        ...
        console.info('upload error undefined type ')
      }
    }
  },
  watch: {
...
.content-list_handlePage_wrapper {
