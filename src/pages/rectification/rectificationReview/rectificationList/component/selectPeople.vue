            ...
            <div class="org">
              ...
              <span>
                <el-checkbox
                  ...
                  :disabled="!item.checked && !setDisable"
                  :label="index"
...
export default {
  ...
  methods: {
    ...
    sortNodeClick(org) {
      if (!org.child || (org.child && org.child.length === 0)) {
        const peopleList = this.selectPeopleList.get(org.id + 1);
    ...
    sure() {
      ...
      if (this.selectedPeople.length >= 2) {
        this.$emit("getPeople", true, this.info);
        //  saveRectificationReview(this.info)
        // .then(res => {
        //   this.$message({
        //     type: "success",
        //     message: "转办成功！"
    ...
    // 人员选择回显
    selectPeople(val) {
      const dataList = this.selectPeopleList.get(
        this.selectedDeptId + this.currPage
      );
      const id = this.selectedDeptId + this.currPage;
      if (!val.checked) {
        ...
        const flag = dataList.children.some(item => {
  ...
  computed: {
    setDisable() {
      ...
      return this.selectedPeople.length < this.selectNum;
    }
  }
};
</script>
